...
const connectDB = async () => {
  ...
  try {
    ...
    try {
      await mongoose.connect('mongodb://127.0.0.1:27017/coldchain', { useUnifiedTopology: true, useNewUrlParser: true, useCreateIndex: true });
      console.log("MongoDB connected (local).");
    } catch (err2) {
      console.warn("Local MongoDB unreachable:", err2.message);
      console.log("Starting in-memory MongoDB...");
      const { MongoMemoryServer } = require('mongodb-memory-server');
      const mongod = await MongoMemoryServer.create({ instance: { dbName: 'coldchain' } });
      uri = mongod.getUri();
      await mongoose.connect(uri, { useUnifiedTopology: true, useNewUrlParser: true, useCreateIndex: true });
...
connectDB().catch(err => {
  ...
  console.warn("Server will continue; IoT may use in-memory fallback.");
});
